...
public class JettyServerHttpErrorHandler extends ErrorHandler {
    ...
    protected void generateAcceptableResponse(ServletContextRequest baseRequest, HttpServletRequest request, HttpServletResponse response, int code, String message) throws IOException {
        ...
        if (throwable instanceof ApplicationRuntimeException applicationRuntimeException) {
            ...
            cause = applicationRuntimeException.getCause();
        ...
        } else if (throwable instanceof ApplicationIOException applicationIOException) {
            statusCode = applicationIOException.getHttpStatus();
            message = applicationIOException.getMessage();
            cause = applicationIOException.getCause();
